...
def setupSelectionStringFieldProperties(portal):
    name = 'PFGSelectionStringField'
    ...
    allowed_content_types = list(
        types['FormFolder'].allowed_content_types
    )
    if name not in allowed_content_types:
        allowed_content_types.append(name)
    types.getTypeInfo('FormFolder').manage_changeProperties(
...
def setupVarious(context):
    ...
    if context.readDataFile(
        'Products.PFGSelectionStringField_various.txt'
    ) is None:
        return

    portal = context.getSite()
    setupSelectionStringFieldProperties(portal)
